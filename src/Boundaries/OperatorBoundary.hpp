...
void OperatorBoundary::exec( Grid& grid, Timer& timer){
    int bound  = grid.buffer_size();
    int startX = grid.state_start(DIM_X);
    int startY = grid.state_start(DIM_Y);
    int endX   = grid.state_end(DIM_X);
    int endY   = grid.state_end(DIM_Y);
    double t = timer.t();
    double levelset;
