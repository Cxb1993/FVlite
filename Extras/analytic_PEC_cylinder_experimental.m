...
bessel=zeros(1,2*N_bessel+2);
hankel=zeros(1,2*N_bessel+2);

for ii=1:2*N_bessel+3
    bessel(ii)=besselj(ii-N_bessel-2,rho);
    hankel(ii)=besselh(ii-N_bessel-2,2,rho);
endfor
...
for ii = 1:N_points
    for jj = 1:2*N_bessel+1
        kk=jj-1-N_bessel;
        bessel_term = 1.0/hankel_der(jj);
        H(ii) = H(ii) + (2j*H_0/(pi*rho)) * (1j)^(-kk) * bessel_term * exp(1j*kk*phi(ii)); 
    endfor
endfor
